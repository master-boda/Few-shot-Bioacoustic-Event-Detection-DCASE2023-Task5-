#!/usr/bin/env python3
"""Grid search for eval.samples_neg and eval.iterations with fixed threshold.

Runs eval with sweep disabled and evaluates using the official metric script.
...
def main() -> None:
    ...
    parser.add_argument("--samples_neg", default="30,50,100")
    parser.add_argument("--iterations", default="3,6,10")
    args = parser.parse_args()

    samples_list = _parse_list(args.samples_neg, int)
    iterations_list = _parse_list(args.iterations, int)
    if not samples_list or not iterations_list:
        raise ValueError("samples_neg and iterations must be non-empty lists")

    outputs_dir = args.outputs_dir
    os.makedirs(outputs_dir, exist_ok=True)

    grid_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    grid_dir = os.path.join(outputs_dir, f"grid_stability_{grid_stamp}")
    ...
    results = []
    total = len(samples_list) * len(iterations_list)
    idx = 0

    for samples_neg in samples_list:
        for iterations in iterations_list:
            idx += 1
            before = _list_run_dirs(run_outputs_dir)
            tag = f"st_neg{samples_neg}_it{iterations}"
            cmd = [
                "python",
                "main.py",
                "set.features=false",
                ...
                f"eval.samples_neg={samples_neg}",
                f"eval.iterations={iterations}",
                f"+eval.run_tag={tag}",
            ]

            print(f"[{idx}/{total}] samples_neg={samples_neg} iterations={iterations}")
    ...
    print(f"samples_neg={best['samples_neg']} iterations={best['iterations']}")
