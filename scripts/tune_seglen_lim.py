#!/usr/bin/env python3
"""Grid search for eval.test_seglen_len_lim with fixed threshold."""
...
def main() -> None:
    ...
    parser.add_argument("--seglen_lims", default="20,30,40")
    parser.add_argument("--samples_neg", type=int, default=30)
    parser.add_argument("--iterations", type=int, default=6)
    parser.add_argument("--hop_factor", type=int, default=2)
    args = parser.parse_args()

    seglen_lims = _parse_list(args.seglen_lims, int)
    if not seglen_lims:
        raise ValueError("seglen_lims must be a non-empty list")

    outputs_dir = args.outputs_dir
    os.makedirs(outputs_dir, exist_ok=True)

    grid_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    grid_dir = os.path.join(outputs_dir, f"grid_seglen_{grid_stamp}")
    ...
    results = []
    total = len(seglen_lims)
    idx = 0

    for seglen_lim in seglen_lims:
        idx += 1
        before = _list_run_dirs(run_outputs_dir)
        tag = f"seglen{seglen_lim}"
        cmd = [
            "python",
            "main.py",
            "set.features=false",
            ...
            f"eval.test_hoplen_fenmu={args.hop_factor}",
            f"eval.test_seglen_len_lim={seglen_lim}",
            f"+eval.run_tag={tag}",
        ]

        print(f"[{idx}/{total}] test_seglen_len_lim={seglen_lim}")
        ...
        results.append(
            {
                "test_seglen_len_lim": seglen_lim,
                "threshold": args.threshold,
                "samples_neg": args.samples_neg,
                "iterations": args.iterations,
                "test_hoplen_fenmu": args.hop_factor,
                "f1": f1,
                "precision": precision,
                "recall": recall,
                "run_dir": run_dir,
    ...
    print(f"test_seglen_len_lim={best['test_seglen_len_lim']}")
